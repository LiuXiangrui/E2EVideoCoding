...
from Common.Utils import Record, DecodedBuffer, calculate_bpp, cal_psnr, separate_aux_and_normal_params
        ...
        for sequence in tqdm(self.train_dataloader, total=len(self.train_dataloader), smoothing=0.9, ncols=50):
            ...
            if self.train_steps % 100 == 0:
                self.tensorboard.add_images(tag="Training/Reconstruction", global_step=self.train_steps,
                                            img_tensor=torch.stack(encode_results["reconstruction"], dim=1)[
                                                0].clone().detach().cpu())
                self.tensorboard.add_images(tag="Training/Pristine", global_step=self.train_steps,
                                            img_tensor=torch.stack(encode_results["pristine"], dim=1)[
                                                0].clone().detach().cpu())
...
if __name__ == "__main__":
    trainer = TrainerDCVC(inter_frame_codec=InterFrameCodecDVC)
    trainer.train()
