...
def parse_encoder_results(results_path: str) -> dict:
    resolution = int(results_path.split('_')[-2].split('x')[0]) * int(results_path.split('_')[-2].split('x')[1])
    frame_rate = int(os.path.splitext(results_path)[0].split('_')[-1])
