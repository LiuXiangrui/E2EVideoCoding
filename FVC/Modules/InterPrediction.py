...
class PredRefine(nn.Module):
    ...
    def forward(self, aligned_ref: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
        return self.net(torch.cat([ref, aligned_ref], dim=1))


class MotionCompensation(nn.Module):
    def __init__(self, group: int = 8, deform_kernel_size: int = 3):
        super().__init__()
        self.deform_offset_est = nn.Conv2d(in_channels=64, out_channels=2 * deform_kernel_size ** 2 * group, kernel_size=3, stride=1, padding=1)
        self.deform_conv = DeformConv2d(in_channels=64, out_channels=64, groups=group, kernel_size=deform_kernel_size, stride=1, padding=1)
